...
class Product{
  ...
  processOrder(){
    ...
    for(let paramId in thisProduct.data.params){                                                // for every category (param)...
      ...
      for(let optionId in param.options){                                                       // for every option in this category
        ...
        const optionImage = thisProduct.imageWrapper.querySelector('.' + paramId + '-' + optionId); // Wybór i zmiana składników -> widoczna zmiana przy wybieraniu
  ...
  addToCart(){
    ...
    const event = new CustomEvent('add-to-cart', {
      bubbles: true,
      detail: {
        product: thisProduct.prepareCartProduct()                                                 // Fix koszyka, wybrany został poprawny element
      },
    });
